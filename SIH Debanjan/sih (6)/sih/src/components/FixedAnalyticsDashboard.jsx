...
const AnalyticsDashboard = ({ user, grievances, workers, onAssignTask }) => {
  ...
  const fetchAnalyticsData = async () => {
    ...
    try {
      ...
      if (!user || !user.token) {
        console.error('No auth token available for analytics');
        throw new Error('Authentication error. Please login again.');
      }
      
      console.log('Fetching analytics with token:', user.token);
      
      // Fetch analytics data from backend with proper error handling
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/analytics/analytics?timeRange=${selectedTimeRange}&metric=${selectedMetric}`,
        {
          method: 'GET',
          headers: {
  ...
  const calculateWorkerEfficiency = async () => {
    try {
      ...
      if (!user || !user.token) {
        console.error('No auth token available for worker efficiency');
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error('Worker efficiency API error:', response.status, errorText);
        throw new Error(`Failed to fetch worker efficiency data: ${response.status}`);
      }
      
      const data = await response.json();
      console.log('Worker efficiency data fetched successfully:', data);
